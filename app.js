const state = {
    user: localStorage.getItem("user"),
    ads: JSON.parse(localStorage.getItem("ads")||"[]"),
    search:""
};

function save(){
    localStorage.setItem("ads", JSON.stringify(state.ads));
}

function show(id){
    document.querySelectorAll(".container")
    .forEach(d=>d.classList.add("hidden"));

    document.getElementById(id).classList.remove("hidden");
}

function login(){
    const user = userInput.value || document.getElementById("user").value;

    state.user = user;
    localStorage.setItem("user", user);

    alert("შესული ხარ");
    render();
    show("home");
}

function createAd(){

    if(!state.user) return alert("შესვლა საჭიროა");

    const images = img.value.split(",");

    const myAds = state.ads.filter(a=>a.user===state.user);

    const vip = document.getElementById("vip").checked;

    if(!vip && myAds.length>=5) return alert("VIP საჭიროა");

    if(!vip && images.length>7) return alert("7 ფოტო მაქს");

    if(vip && images.length>14) return alert("14 ფოტო მაქს");

    state.ads.push({
        id:Date.now(),
        title:title.value,
        desc:desc.value,
        images,
        vip,
        user:state.user,
        date:Date.now()
    });

    save();
    render();
    show("home");
}

function deleteAd(id){
    state.ads = state.ads.filter(a=>a.id!==id);
    save();
    render();
}

function renderAds(){

    const list = state.ads
    .filter(a=>a.title.toLowerCase().includes(state.search));

    ads.innerHTML = list.map(a=>`
    <div class="ad ${a.vip?'vip':''}">
        <img src="${a.images[0]||''}">
        <h3>${a.title}</h3>
        <p>${a.desc}</p>
    </div>`).join("");
}

function renderCabinet(){

    const my = state.ads.filter(a=>a.user===state.user);

    myAds.innerHTML = my.map(a=>`
    <div class="ad ${a.vip?'vip':''}">
        <h3>${a.title}</h3>
        <button onclick="deleteAd(${a.id})">წაშლა</button>
    </div>`).join("");
}

function render(){
    renderAds();
    renderCabinet();
}

search.oninput = e=>{
    state.search = e.target.value.toLowerCase();
    render();
};

function sendMsg(){
    chatBox.innerHTML += `<div>${state.user||"Guest"}: ${chatInput.value}</div>`;
    chatInput.value="";
}

// AUTO DELETE 30 DAYS
setInterval(()=>{
    const now = Date.now();
    state.ads = state.ads.filter(a=> now-a.date < 2592000000);
    save();
    render();
},60000);

window.onload = render;
